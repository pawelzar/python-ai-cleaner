...
from core.algorithm import a_star_search, heuristic, path_as_orders
from core.neuron import Neuron
from extra.draw import draw_grid
from extra.fuzzy import (
    fuzzy_battery, fuzzy_container, fuzzy_distance, fuzzy_soap
)
from genetic.optimize import optimize_route
from object import Object
from settings import (
    CROSSOVER_CHANCE, GENERATIONS, GRID_HEIGHT, GRID_WIDTH,
    MAX_BATTERY, MAX_SOAP, MUTATION_CHANCE, POPULATION
)
...
class Cleaner(Object):
    ...
    def initialize(self, board, screen, network, classification, images):
        self.board = board
        self.screen = screen
    ...
    def decide_to_clean(self, position, item):
        ...
        """
        instance = item  # name of the object (type of dirt)
        ...
        result = self.classification.classify(distance, instance, soap,
                                              battery, container)
        print(
            'distance: {}, type: {}, soap level: {}, battery: {}, '
            'container: {}, decision: {}'.format(
                *map(str.upper, [
                    distance, instance, soap, battery, container, result
                ])
            )
        )
    ...
    def decide_to_refill(self):
        ...
        print(
            'distance to station: {}, distance to bin: {}, battery: {}, '
            'soap: {}, container: {}, decision: {}'.format(
                *map(str.upper, [
                    dist_sta, dist_bin, battery, soap, container, result
                ])
            )
        )
    ...
    def decide_and_clean(self):
        ...
        if self.data:
            ...
            for position, item in sorted(self.data.items(), key=closest):
                # use item = self.data[position]
                # if iterate through optimal path
                result = self.decide_to_clean(position, item)
                if result == 'True':
                    self.move_to(position, False, False)
                    self.clean()
                    # use self.optimal_path.remove(position)
                    # if iterate through optimal path
                    break
            else:
                # if cleaner is not able to clean any object
    ...
    def generate_and_clean(self, amount):
        """
        Generate new random dirt on board and automatically set agent to
        clean all.
        """
    ...
    def optimize_path(self):
        ...
        """
        self.optimal_path = optimize_route(
            self.data.keys(), GENERATIONS, CROSSOVER_CHANCE,
