def draw_point(graph, point, style):
    ...
    """
    sign = '-'
    if point in graph.objects:
        sign = graph.objects[point]
    if point in graph.obstacles:
        sign = graph.obstacles[point]
    if point == graph.station.position:
        sign = 'S'
    if point == graph.agent.position:
        sign = 'A'
    if point == graph.basket.position:
        sign = 'B'
